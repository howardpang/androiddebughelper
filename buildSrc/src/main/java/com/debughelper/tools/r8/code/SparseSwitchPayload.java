...
public class SparseSwitchPayload extends SwitchPayload {
  ...
  public SparseSwitchPayload(int high, BytecodeStream stream) {
    super(high, stream);
    size = com.debughelper.tools.r8.code.Instruction.read16BitValue(stream);
    keys = new int[size];
    for (int i = 0; i < size; i++) {
      keys[i] = com.debughelper.tools.r8.code.Instruction.readSigned32BitValue(stream);
    }

    targets = new int[size];
    for (int i = 0; i < size; i++) {
      targets[i] = com.debughelper.tools.r8.code.Instruction.readSigned32BitValue(stream);
    }
  }
  ...
  @Override
  public String toString(ClassNameMapper naming, com.debughelper.tools.r8.code.Instruction payloadUser) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (payloadUser != null) {
        ...
        offsetString = com.debughelper.tools.r8.utils.StringUtils.hexString(targets[i] + payloadUser.getOffset(), 2);
      } else {
        offsetString = targets[i] >= 0 ? ("+" + targets[i]) : Integer.toString(targets[i]);
      }
      com.debughelper.tools.r8.utils.StringUtils.appendLeftPadded(builder, keys[i] + " -> " + offsetString + "\n", 20);
    }
