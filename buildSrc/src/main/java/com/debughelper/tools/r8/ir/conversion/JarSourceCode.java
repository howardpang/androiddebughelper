...
import com.debughelper.tools.r8.ir.conversion.JarState.Local;
import com.debughelper.tools.r8.ir.conversion.JarState.Slot;
...
public class JarSourceCode implements SourceCode {
  ...
  private final com.debughelper.tools.r8.graph.DexType clazz;
  ...
  private com.debughelper.tools.r8.ir.code.Monitor monitorEnter = null;
  ...
  private com.debughelper.tools.r8.ir.code.Position currentPosition;

  // Canonicalized positions to lower memory usage.
  private final Int2ReferenceMap<com.debughelper.tools.r8.ir.code.Position> canonicalPositions = new Int2ReferenceOpenHashMap<>();

  // Synthetic position with line = 0.
  private com.debughelper.tools.r8.ir.code.Position preamblePosition = null;

  // Cooked position to indicate positions in synthesized code (ie, for synchronization).
  private com.debughelper.tools.r8.ir.code.Position syntheticPosition = null;

  private final com.debughelper.tools.r8.graph.DexMethod method;
  private final com.debughelper.tools.r8.ir.code.Position callerPosition;

  public JarSourceCode(
      com.debughelper.tools.r8.graph.DexType clazz,
      MethodNode node,
  ...
  public void setUp() {
    if (com.debughelper.tools.r8.logging.Log.ENABLED) {
      com.debughelper.tools.r8.logging.Log.debug(JarSourceCode.class, "Computing blocks for:\n" + toString());
    }
  }
  ...
  public void buildPrelude(IRBuilder builder) {
    ...
    Int2ReferenceMap<com.debughelper.tools.r8.ir.code.ValueType> argumentLocals = recordArgumentTypes();
    Int2ReferenceMap<com.debughelper.tools.r8.ir.code.ValueType> initializedLocals = new Int2ReferenceOpenHashMap<>(argumentLocals);
    Int2ReferenceMap<com.debughelper.tools.r8.ir.code.ValueType> uninitializedLocals = new Int2ReferenceOpenHashMap<>();
    ...
    for (Object o : node.localVariables) {
      ...
      Type localType;
      com.debughelper.tools.r8.ir.code.ValueType localValueType;
      switch (application.getAsmType(local.desc).getSort()) {
        case Type.OBJECT:
        case Type.ARRAY: {
          localType = JarState.NULL_TYPE;
          localValueType = com.debughelper.tools.r8.ir.code.ValueType.OBJECT;
          break;
        }
        case Type.LONG: {
          localType = Type.LONG_TYPE;
          localValueType = com.debughelper.tools.r8.ir.code.ValueType.LONG;
          break;
        }
        case Type.DOUBLE: {
          localType = Type.DOUBLE_TYPE;
          localValueType = com.debughelper.tools.r8.ir.code.ValueType.DOUBLE;
          break;
        }
        case Type.BOOLEAN:
        case Type.CHAR:
        ...
        case Type.INT: {
          ...
          localValueType = com.debughelper.tools.r8.ir.code.ValueType.INT;
          break;
        }
        case Type.FLOAT: {
          localType = Type.FLOAT_TYPE;
          localValueType = com.debughelper.tools.r8.ir.code.ValueType.FLOAT;
          break;
        }
        case Type.VOID:
        case Type.METHOD:
        default:
          throw new com.debughelper.tools.r8.errors.Unreachable("Invalid local variable type: " );
      }
      int localRegister = state.getLocalRegister(local.index, localType);
      com.debughelper.tools.r8.ir.code.ValueType existingLocalType = initializedLocals.get(localRegister);
    ...
    for (Entry<com.debughelper.tools.r8.ir.code.ValueType> entry : uninitializedLocals.int2ReferenceEntrySet()) {
      builder.addDebugUninitialized(entry.getIntKey(), entry.getValue());
    }

    // Add debug information for all locals at the initial label.
    for (JarState.Local local : state.getLocalsToOpen()) {
    ...
    if (generateMethodSynchronization()) {
      ...
      monitorEnter = builder.addMonitor(com.debughelper.tools.r8.ir.code.Monitor.Type.ENTER, monitorRegister);
  ...
  private void buildArgumentInstructions(IRBuilder builder) {
    ...
    if (!isStatic()) {
      ...
      JarState.Slot slot = state.readLocal(argumentRegister++, thisType);
      builder.addThisArgument(slot.register);
    }
    for (Type type : parameterTypes) {
      com.debughelper.tools.r8.ir.code.ValueType valueType = valueType(type);
      JarState.Slot slot = state.readLocal(argumentRegister, type);
  ...
  private Int2ReferenceMap<com.debughelper.tools.r8.ir.code.ValueType> recordArgumentTypes() {
    Int2ReferenceMap<com.debughelper.tools.r8.ir.code.ValueType> initializedLocals =
  ...
  private void computeBlockEntryJarStates(IRBuilder builder) {
    ...
    IRBuilder.BlockInfo entry = CFG.get(IRBuilder.INITIAL_BLOCK_OFFSET);
    if (CFG.get(0) != null) {
      entry = CFG.get(0);
    }
  ...
  private void buildMonitorExit(IRBuilder builder) {
    ...
    builder.add(new com.debughelper.tools.r8.ir.code.Monitor(com.debughelper.tools.r8.ir.code.Monitor.Type.EXIT, monitorEnter.inValues().get(0)));
  }
  ...
  public void buildInstruction(
    ...
    if (hasNextInstruction || !isControlFlowInstruction(insn)) {
      ...
      for (JarState.Local local : state.getLocalsToClose()) {
    ...
    if (com.debughelper.tools.r8.logging.Log.ENABLED && !(insn instanceof LineNumberNode)) {
      int offset = getOffset(insn);
      if (insn instanceof LabelNode) {
        com.debughelper.tools.r8.logging.Log.debug(getClass(), "\n%4d: %s",
  ...
  public boolean verifyLocalInScope(DebugLocalInfo local) {
    for (JarState.Local open : state.getLocals()) {
  ...
  private List<com.debughelper.tools.r8.graph.DexType> getTryHandlerGuards(List<TryCatchBlock> tryCatchBlocks) {
  ...
  private int[] getJumpTargets(JumpInsnNode jump) {
    switch (jump.getOpcode()) {
      ...
      case Opcodes.JSR: {
        throw new com.debughelper.tools.r8.errors.Unreachable("JSR should be handled by the ASM jsr inliner");
      }
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected opcode in jump instruction: " + jump);
    }
  }

  private int[] getVarTargets(VarInsnNode insn) {
    if (insn.getOpcode() == Opcodes.RET) {
      throw new com.debughelper.tools.r8.errors.Unreachable("RET should be handled by the ASM jsr inliner");
    }
    return NO_TARGETS;
  }
  ...
  private static com.debughelper.tools.r8.ir.code.ValueType valueType(Type type) {
    switch (type.getSort()) {
      case Type.ARRAY:
      case Type.OBJECT:
        return com.debughelper.tools.r8.ir.code.ValueType.OBJECT;
      case Type.BOOLEAN:
      case Type.BYTE:
      case Type.SHORT:
      case Type.CHAR:
      case Type.INT:
        return com.debughelper.tools.r8.ir.code.ValueType.INT;
      case Type.FLOAT:
        return com.debughelper.tools.r8.ir.code.ValueType.FLOAT;
      case Type.LONG:
        return com.debughelper.tools.r8.ir.code.ValueType.LONG;
      case Type.DOUBLE:
        return com.debughelper.tools.r8.ir.code.ValueType.DOUBLE;
      case Type.VOID:
        // Illegal. Throws in fallthrough.
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Invalid type in valueType: " + type);
    }
  }

  private static com.debughelper.tools.r8.ir.code.MemberType memberType(Type type) {
    switch (type.getSort()) {
      case Type.ARRAY:
      case Type.OBJECT:
      ...
      case Type.BYTE:
        return com.debughelper.tools.r8.ir.code.MemberType.BYTE;
      case Type.SHORT:
        return com.debughelper.tools.r8.ir.code.MemberType.SHORT;
      case Type.CHAR:
      ...
      case Type.VOID:
        // Illegal. Throws in fallthrough.
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Invalid type in memberType: " + type);
    }
  }
  ...
  private static com.debughelper.tools.r8.ir.code.NumericType numericType(Type type) {
    switch (type.getSort()) {
      case Type.BYTE:
        return com.debughelper.tools.r8.ir.code.NumericType.BYTE;
      case Type.CHAR:
        return com.debughelper.tools.r8.ir.code.NumericType.CHAR;
      case Type.SHORT:
        return com.debughelper.tools.r8.ir.code.NumericType.SHORT;
      case Type.INT:
        return com.debughelper.tools.r8.ir.code.NumericType.INT;
      case Type.LONG:
        return com.debughelper.tools.r8.ir.code.NumericType.LONG;
      case Type.FLOAT:
        return com.debughelper.tools.r8.ir.code.NumericType.FLOAT;
      case Type.DOUBLE:
        return com.debughelper.tools.r8.ir.code.NumericType.DOUBLE;
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Invalid type in numericType: " + type);
    }
  }

  private com.debughelper.tools.r8.ir.code.Invoke.Type invokeType(MethodInsnNode method) {
    switch (method.getOpcode()) {
      case Opcodes.INVOKEVIRTUAL:
        if (isCallToPolymorphicSignatureMethod(method)) {
          return com.debughelper.tools.r8.ir.code.Invoke.Type.POLYMORPHIC;
        }
        return com.debughelper.tools.r8.ir.code.Invoke.Type.VIRTUAL;
      case Opcodes.INVOKESTATIC:
        return com.debughelper.tools.r8.ir.code.Invoke.Type.STATIC;
      case Opcodes.INVOKEINTERFACE:
        return com.debughelper.tools.r8.ir.code.Invoke.Type.INTERFACE;
      case Opcodes.INVOKESPECIAL: {
        ...
        com.debughelper.tools.r8.graph.DexType owner = application.getTypeFromName(method.owner);
        if (owner == clazz || method.name.equals(Constants.INSTANCE_INITIALIZER_NAME)) {
          ...
          return com.debughelper.tools.r8.ir.code.Invoke.Type.SUPER;
        }
      }
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected MethodInsnNode opcode: " + method.getOpcode());
    }
  }
  ...
  private static boolean isCompatibleArrayElementType(int opcode, Type type) {
    switch (opcode) {
      ...
      case Opcodes.BALOAD:
      case Opcodes.BASTORE:
        return JarState.Slot.isCompatible(type, Type.BYTE_TYPE)
            || JarState.Slot.isCompatible(type, Type.BOOLEAN_TYPE);
      case Opcodes.CALOAD:
      case Opcodes.CASTORE:
  ...
  private static com.debughelper.tools.r8.ir.code.If.Type ifType(int opcode) {
    switch (opcode) {
      ...
      case Opcodes.IFLE:
      case Opcodes.IF_ICMPLE:
        return com.debughelper.tools.r8.ir.code.If.Type.LE;
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected If instruction opcode: " + opcode);
    }
  }
  ...
  private static Type opType(int opcode) {
    switch (opcode) {
      ...
      case Opcodes.DNEG:
        return Type.DOUBLE_TYPE;
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected opcode " + opcode);
    }
  }
  ...
  private void updateState(AbstractInsnNode insn) {
    switch (insn.getType()) {
      ...
      case AbstractInsnNode.LINE:
        ...
        break;
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected instruction " + insn);
    }
  }
    ...
    switch (opcode) {
      ...
      case Opcodes.POP2: {
        ...
        if (value.isCategory1()) {
          JarState.Slot value2 = state.pop();
          assert value2.isCategory1();
        }
        break;
      }
      case Opcodes.DUP: {
        JarState.Slot value = state.peek();
      ...
      case Opcodes.DUP_X1: {
        ...
        JarState.Slot value1 = state.pop();
        JarState.Slot value2 = state.pop();
      ...
      case Opcodes.DUP_X2: {
        ...
        if (value2.isCategory1()) {
          JarState.Slot value3 = state.pop();
      ...
      case Opcodes.DUP2_X2: {
        ...
        if (!value1.isCategory1() && !value2.isCategory1()) {
          ...
          if (!value1.isCategory1()) {
            ...
            JarState.Slot value4 = state.pop();
            assert value1.isCategory1();
            assert value2.isCategory1();
            assert value3.isCategory1();
      ...
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected Insn opcode: " + insn.getOpcode());
    }
  }
  ...
  private void updateState(IntInsnNode insn) {
    switch (insn.getOpcode()) {
      ...
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected IntInsn opcode: " + insn.getOpcode());
    }
  }
  ...
  private void updateState(VarInsnNode insn) {
    ...
    switch (opcode) {
      ...
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected VarInsn opcode: " + insn.getOpcode());
    }
    if (Opcodes.ILOAD <= opcode && opcode <= Opcodes.ALOAD) {
      JarState.Slot src = state.readLocal(insn.var, expectedType);
      state.push(src.type);
    } else {
      assert Opcodes.ISTORE <= opcode && opcode <= Opcodes.ASTORE;
      JarState.Slot slot = state.pop();
  ...
  private void updateState(TypeInsnNode insn) {
    ...
    switch (insn.getOpcode()) {
      ...
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected TypeInsn opcode: " + insn.getOpcode());
    }
  ...
  private void updateState(FieldInsnNode insn) {
    ...
    switch (insn.getOpcode()) {
      ...
      default:
        throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected FieldInsn opcode: " + insn.getOpcode());
    }
  }
  ...
  private void updateState(JumpInsnNode insn) {
    ...
    if (Opcodes.IFEQ <= opcode && opcode <= Opcodes.IF_ACMPNE) {
      ...
      switch (opcode) {
        ...
        default:
          throw new com.debughelper.tools.r8.errors.Unreachable("Unexpected JumpInsn opcode: " + insn.getOpcode());
      }
    }
  }
  ...
  private void updateState(LdcInsnNode insn) {
    if (insn.cst instanceof Type) {
      ...
      throw new com.debughelper.tools.r8.errors.CompilationError("Unsupported constant: " + insn.cst.toString());
    }
  }
  ...
  private void processLocalVariablesAtControlEdge(AbstractInsnNode insn, IRBuilder builder) {
    ...
    IRBuilder.BlockInfo blockInfo = builder.getCFG().get(blockOffset);
    // Read all locals that are not live on all successors to ensure liveness.
    for (JarState.Local local : state.localsNotLiveAtAllSuccessors(blockInfo.allSuccessors())) {
  ...
  private void processLocalVariablesAtExit(AbstractInsnNode insn, IRBuilder builder) {
    ...
    for (JarState.Local local : state.getLocals()) {
      if (local.info != null) {
        builder.addDebugLocalRead(local.slot.register, local.info);
      }
    }
  ...
  private void build(InsnNode insn, IRBuilder builder) {
    ...
    switch (opcode) {
      ...
      case Opcodes.CALOAD:
      case Opcodes.SALOAD: {
        JarState.Slot index = state.pop(Type.INT_TYPE);
        JarState.Slot array = state.pop(JarState.ARRAY_TYPE);
      ...
      case Opcodes.DREM: {
        Type type = opType(opcode);
        com.debughelper.tools.r8.ir.code.NumericType numericType = numericType(type);
      ...
      case Opcodes.I2S:
        ...
        break;
      case Opcodes.LCMP: {
        JarState.Slot right = state.pop(Type.LONG_TYPE);
        JarState.Slot left = state.pop(Type.LONG_TYPE);
        int dest = state.push(Type.INT_TYPE);
        builder.addCmp(com.debughelper.tools.r8.ir.code.NumericType.LONG, Cmp.Bias.NONE, dest, left.register, right.register);
        break;
      }
      case Opcodes.FCMPL:
      case Opcodes.FCMPG: {
        JarState.Slot right = state.pop(Type.FLOAT_TYPE);
        JarState.Slot left = state.pop(Type.FLOAT_TYPE);
        int dest = state.push(Type.INT_TYPE);
        Cmp.Bias bias = opcode == Opcodes.FCMPL ? Cmp.Bias.LT : Cmp.Bias.GT;
        builder.addCmp(com.debughelper.tools.r8.ir.code.NumericType.FLOAT, bias, dest, left.register, right.register);
        break;
      }
      case Opcodes.DCMPL:
      case Opcodes.DCMPG: {
        JarState.Slot right = state.pop(Type.DOUBLE_TYPE);
        JarState.Slot left = state.pop(Type.DOUBLE_TYPE);
        int dest = state.push(Type.INT_TYPE);
        Cmp.Bias bias = opcode == Opcodes.DCMPL ? Cmp.Bias.LT : Cmp.Bias.GT;
        builder.addCmp(com.debughelper.tools.r8.ir.code.NumericType.DOUBLE, bias, dest, left.register, right.register);
        break;
      }
      case Opcodes.IRETURN: {
        JarState.Slot value = state.pop(Type.INT_TYPE);
        addReturn(insn, com.debughelper.tools.r8.ir.code.ValueType.INT, value.register, builder);
        break;
      }
      case Opcodes.LRETURN: {
        JarState.Slot value = state.pop(Type.LONG_TYPE);
        addReturn(insn, com.debughelper.tools.r8.ir.code.ValueType.LONG, value.register, builder);
        break;
      }
      case Opcodes.FRETURN: {
        JarState.Slot value = state.pop(Type.FLOAT_TYPE);
        addReturn(insn, com.debughelper.tools.r8.ir.code.ValueType.FLOAT, value.register, builder);
        break;
      }
      case Opcodes.DRETURN: {
        JarState.Slot value = state.pop(Type.DOUBLE_TYPE);
        addReturn(insn, com.debughelper.tools.r8.ir.code.ValueType.DOUBLE, value.register, builder);
        break;
      }
      case Opcodes.ARETURN: {
        JarState.Slot obj = state.pop(JarState.REFERENCE_TYPE);
        addReturn(insn, com.debughelper.tools.r8.ir.code.ValueType.OBJECT, obj.register, builder);
        break;
      }
      case Opcodes.RETURN: {
      ...
      case Opcodes.ATHROW: {
        JarState.Slot object = state.pop(JarState.OBJECT_TYPE);
        addThrow(insn, object.register, builder);
        break;
      }
      case Opcodes.MONITORENTER: {
        JarState.Slot object = state.pop(JarState.REFERENCE_TYPE);
        builder.addMonitor(com.debughelper.tools.r8.ir.code.Monitor.Type.ENTER, object.register);
        break;
      }
      case Opcodes.MONITOREXIT: {
        JarState.Slot object = state.pop(JarState.REFERENCE_TYPE);
  ...
  private void addReturn(InsnNode insn, com.debughelper.tools.r8.ir.code.ValueType type, int register, IRBuilder builder) {
    processLocalVariablesAtExit(insn, builder);
    if (type == null) {
      assert register == -1;
      builder.addReturn();
  ...
  private void dupTwoBelowTwo(JarState.Slot value4, JarState.Slot value3, JarState.Slot value2, JarState.Slot value1,
                              IRBuilder builder) {
  ...
  private void build(IntInsnNode insn, IRBuilder builder) {
    switch (insn.getOpcode()) {
      ...
      case Opcodes.NEWARRAY: {
        String desc = arrayTypeDesc(insn.operand);
        Type type = application.getAsmType(desc);
        com.debughelper.tools.r8.graph.DexType dexType = application.getTypeFromDescriptor(desc);
  ...
  private void build(VarInsnNode insn, IRBuilder builder) {
    ...
    if (Opcodes.ILOAD <= opcode && opcode <= Opcodes.ALOAD) {
      ...
      assert Opcodes.ISTORE <= opcode && opcode <= Opcodes.ASTORE;
      JarState.Slot src = state.pop(expectedType);
  ...
  private void build(TypeInsnNode insn, IRBuilder builder) {
    ...
    switch (insn.getOpcode()) {
      case Opcodes.NEW: {
        com.debughelper.tools.r8.graph.DexType dexType = application.getTypeFromName(insn.desc);
        int dest = state.push(type);
      ...
      case Opcodes.ANEWARRAY: {
        Type arrayType = makeArrayType(type);
        com.debughelper.tools.r8.graph.DexType dexArrayType = application.getTypeFromDescriptor(arrayType.getDescriptor());
        int count = state.pop(Type.INT_TYPE).register;
      ...
      case Opcodes.CHECKCAST: {
        com.debughelper.tools.r8.graph.DexType dexType = application.getTypeFromDescriptor(type.getDescriptor());
        // Pop the top value and push it back on with the checked type.
        state.pop(type);
  ...
  private void build(FieldInsnNode insn, IRBuilder builder) {
    ...
    switch (insn.getOpcode()) {
      ...
      case Opcodes.PUTFIELD: {
        JarState.Slot value = state.pop(type);
        JarState.Slot object = state.pop(JarState.OBJECT_TYPE);
  ...
  private void build(MethodInsnNode insn, IRBuilder builder) {
    ...
    com.debughelper.tools.r8.graph.DexMethod method = application.getMethod(insn.owner, insn.name, insn.desc);
    ...
    buildInvoke(
        ...
        builder,
        (types, registers) -> {
          com.debughelper.tools.r8.ir.code.Invoke.Type invokeType = invokeType(insn);
          DexProto callSiteProto = null;
          com.debughelper.tools.r8.graph.DexMethod targetMethod = method;
          if (invokeType == com.debughelper.tools.r8.ir.code.Invoke.Type.POLYMORPHIC) {
  ...
  private void buildInvoke(
      ...
      IRBuilder builder,
      BiConsumer<List<com.debughelper.tools.r8.ir.code.ValueType>, List<Integer>> creator) {
    ...
    JarState.Slot[] parameterRegisters = state.popReverse(parameterTypes.length);

    List<com.debughelper.tools.r8.ir.code.ValueType> types = new ArrayList<>(parameterTypes.length + 1);
  ...
  private static void addArgument(List<com.debughelper.tools.r8.ir.code.ValueType> types, List<Integer> registers, Type type,
                                  JarState.Slot slot) {
  ...
  private void build(InvokeDynamicInsnNode insn, IRBuilder builder) {
    com.debughelper.tools.r8.graph.DexCallSite callSite = DexCallSite.fromAsmInvokeDynamic(insn, application, clazz);
  ...
  private void build(JumpInsnNode insn, IRBuilder builder) {
    ...
    if (Opcodes.IFEQ <= opcode && opcode <= Opcodes.IF_ACMPNE) {
      ...
      if (opcode <= Opcodes.IFLE) {
        JarState.Slot value = state.pop(Type.INT_TYPE);
        builder.addIfZero(ifType(opcode), com.debughelper.tools.r8.ir.code.ValueType.INT, value.register, targets[0], targets[1]);
      } else {
        ...
        JarState.Slot value2 = state.pop(expectedType);
        JarState.Slot value1 = state.pop(expectedType);
        builder.addIf(
      ...
      switch (opcode) {
        ...
        case Opcodes.IFNULL:
        case Opcodes.IFNONNULL: {
          JarState.Slot value = state.pop(JarState.REFERENCE_TYPE);
          com.debughelper.tools.r8.ir.code.If.Type type = opcode == Opcodes.IFNULL ? com.debughelper.tools.r8.ir.code.If.Type.EQ : If.Type.NE;
          builder.addIfZero(type, com.debughelper.tools.r8.ir.code.ValueType.OBJECT, value.register, targets[0], targets[1]);
          break;
        }
        case Opcodes.JSR: {
          throw new com.debughelper.tools.r8.errors.Unreachable("JSR should be handled by the ASM jsr inliner");
  ...
  private void build(MultiANewArrayInsnNode insn, IRBuilder builder) {
    // Type of the full array.
    Type arrayType = application.getAsmObjectType(insn.desc);
    com.debughelper.tools.r8.graph.DexType dexArrayType = application.getType(arrayType);
    JarState.Slot[] slots = state.popReverse(insn.dims, Type.INT_TYPE);
    ...
    com.debughelper.tools.r8.graph.DexType memberType = application.getTypeFromDescriptor(insn.desc.substring(insn.dims));
    // Push an array containing the dimensions of the desired multi-dimensional array.
    com.debughelper.tools.r8.graph.DexType dimArrayType = application.getTypeFromDescriptor(INT_ARRAY_DESC);
  ...
  public com.debughelper.tools.r8.ir.code.Position getDebugPositionAtOffset(int offset) {
    ...
    if (index < 0 || instructionCount() <= index) {
      assert false;
      return com.debughelper.tools.r8.ir.code.Position.none();
    }
  ...
  private com.debughelper.tools.r8.ir.code.Position getCanonicalPosition(int line) {
    return canonicalPositions.computeIfAbsent(
        line, l -> new com.debughelper.tools.r8.ir.code.Position(l, null, method, callerPosition));
  }

  private com.debughelper.tools.r8.ir.code.Position getPreamblePosition() {
    if (preamblePosition == null) {
      preamblePosition = com.debughelper.tools.r8.ir.code.Position.synthetic(0, method, null);
    }
  ...
  private com.debughelper.tools.r8.ir.code.Position getExceptionalExitPosition() {
    if (syntheticPosition == null) {
      ...
      syntheticPosition =
          (min == Integer.MAX_VALUE)
              ? com.debughelper.tools.r8.ir.code.Position.noneWithMethod(method, callerPosition)
