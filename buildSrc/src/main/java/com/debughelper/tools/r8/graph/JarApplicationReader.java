...
public class JarApplicationReader {
  ...
  public com.debughelper.tools.r8.graph.DexType getType(Type type) {
    return getTypeFromDescriptor(type.getDescriptor());
  }

  public com.debughelper.tools.r8.graph.DexType getTypeFromName(String name) {
    assert isValidInternalName(name);
    return getType(getAsmObjectType(name));
  }

  public com.debughelper.tools.r8.graph.DexType getTypeFromDescriptor(String desc) {
    assert isValidDescriptor(desc);
    return options.itemFactory.createType(getString(desc));
  }

  public com.debughelper.tools.r8.graph.DexTypeList getTypeListFromNames(String[] names) {
    if (names.length == 0) {
      return com.debughelper.tools.r8.graph.DexTypeList.empty();
    }
    com.debughelper.tools.r8.graph.DexType[] types = new com.debughelper.tools.r8.graph.DexType[names.length];
    for (int i = 0; i < names.length; i++) {
      types[i] = getTypeFromName(names[i]);
    }
    return new com.debughelper.tools.r8.graph.DexTypeList(types);
  }

  public com.debughelper.tools.r8.graph.DexTypeList getTypeListFromDescriptors(String[] descriptors) {
    if (descriptors.length == 0) {
      return com.debughelper.tools.r8.graph.DexTypeList.empty();
    }
    com.debughelper.tools.r8.graph.DexType[] types = new com.debughelper.tools.r8.graph.DexType[descriptors.length];
  ...
  public DexField getField(com.debughelper.tools.r8.graph.DexType owner, String name, String desc) {
  ...
  public com.debughelper.tools.r8.graph.DexProto getProto(String desc) {
    assert isValidDescriptor(desc);
