...
import static com.debughelper.tools.r8.utils.DescriptorUtils.javaTypeToDescriptorIfValidJavaType;
...
public final class IdentifierNameStringUtils {
   ...
   * @param invoke {@link com.debughelper.tools.r8.ir.code.InvokeMethod} that is expected to have an identifier in its arguments.
  ...
  public static DexItemBasedString identifyIdentiferNameString(
      AppInfo appInfo, com.debughelper.tools.r8.ir.code.InvokeMethod invoke) {
    ...
    if (ins.size() == 1) {
      com.debughelper.tools.r8.ir.code.Value in = ins.get(0);
      if (in.isConstString()) {
    ...
    com.debughelper.tools.r8.ir.code.Value in = ins.get(positionOfIdentifier);
    if (!in.isConstString()) {
      return null;
    }
    com.debughelper.tools.r8.ir.code.Value classValue = ins.get(0);
    ...
    if (isReferenceFieldUpdater) {
      com.debughelper.tools.r8.ir.code.Value fieldTypeValue = ins.get(1);
  ...
  /**
   * Visits all {@link com.debughelper.tools.r8.ir.code.ArrayPut}'s with the given {@param classListValue} as array and {@link Class}
  ...
  private static DexTypeList retrieveDexTypeListFromClassList(
    ...
    Set<com.debughelper.tools.r8.ir.code.Instruction> users = classListValue.definition.outValue().uniqueUsers();
    int maxIndex = -1;
