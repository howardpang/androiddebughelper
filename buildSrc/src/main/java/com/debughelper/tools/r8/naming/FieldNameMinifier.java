...
class FieldNameMinifier extends com.debughelper.tools.r8.naming.MemberNameMinifier<DexField, DexType> {
  ...
  private void reserveNamesInSubtypes(DexType type, com.debughelper.tools.r8.naming.NamingState<DexType, ?> state) {
    DexClass holder = appInfo.definitionFor(type);
    if (holder == null) {
      return;
    }
    com.debughelper.tools.r8.naming.NamingState<DexType, ?> newState = computeStateIfAbsent(type, t -> state.createChild());
  ...
  private void reserveFieldName(
      ...
      com.debughelper.tools.r8.naming.NamingState<DexType, ?> state,
      boolean isLibrary) {
  ...
  private void renameFieldsInSubtypes(DexType type) {
    ...
    com.debughelper.tools.r8.naming.NamingState<DexType, ?> state = getState(clazz.type);
    assert state != null;
    clazz.forEachField(field -> renameField(field, state));
    type.forAllExtendsSubtypes(this::renameFieldsInSubtypes);
  }

  private void renameField(DexEncodedField encodedField, com.debughelper.tools.r8.naming.NamingState<DexType, ?> state) {
    DexField field = encodedField.field;
