...
import com.debughelper.tools.r8.naming.MemberNameMinifier;
...
import com.debughelper.tools.r8.utils.MethodSignatureRelaxedEquivalence;
 ...
 * subtyping tree of classes. Such a naming scope is encoded by {@link com.debughelper.tools.r8.naming.NamingState}. It keeps
 ...
 * state separately for each proto using a map from protos to {@link com.debughelper.tools.r8.naming.NamingState.InternalState}
 ...
 * {@link com.debughelper.tools.r8.naming.NamingState} objects for library classes. We can fully allocate these objects as we
 ...
 */
class MethodNameMinifier extends com.debughelper.tools.r8.naming.MemberNameMinifier<DexMethod, DexProto> {
  ...
  private void assignNamesToClassesMethods(DexType type, boolean doPrivates) {
    ...
    if (holder != null && !holder.isLibraryClass()) {
      ...
      com.debughelper.tools.r8.naming.NamingState<DexProto, ?> state =
  ...
  private Set<com.debughelper.tools.r8.naming.NamingState<DexProto, ?>> getReachableStates(DexType type,
                                                                                       Map<DexType, DexType> frontierMap) {
    Set<DexType> interfaces = Sets.newIdentityHashSet();
    interfaces.add(type);
    collectSuperInterfaces(type, interfaces);
    collectSubInterfaces(type, interfaces);
    Set<com.debughelper.tools.r8.naming.NamingState<DexProto, ?>> reachableStates = new HashSet<>();
    for (DexType iface : interfaces) {
      ...
      iface.forAllImplementsSubtypes(t -> {
        com.debughelper.tools.r8.naming.NamingState<DexProto, ?> state = getState(frontierMap.get(t));
        assert state != null;
  ...
  private void assignNamesToInterfaceMethods(Map<DexType, DexType> frontierMap, Timing timing) {
    ...
    Map<Wrapper<DexMethod>, Set<com.debughelper.tools.r8.naming.NamingState<DexProto, ?>>> globalStateMap = new HashMap<>();
    ...
    Map<Wrapper<DexMethod>, com.debughelper.tools.r8.naming.NamingState<DexProto, ?>> originStates = new HashMap<>();
    DexType.forAllInterfaces(appInfo.dexItemFactory, iface -> {
      assert iface.isInterface();
      DexClass clazz = appInfo.definitionFor(iface);
      if (clazz != null) {
        Set<com.debughelper.tools.r8.naming.NamingState<DexProto, ?>> collectedStates = getReachableStates(iface, frontierMap);
    ...
    for (Wrapper<DexMethod> key : methods) {
      ...
      for (Wrapper<DexMethod> k : unification.getOrDefault(key, Collections.singleton(key))) {
        ...
        for (com.debughelper.tools.r8.naming.NamingState<DexProto, ?> namingState : globalStateMap.get(k)) {
          collectedStates.add(
  ...
  private void addStatesToGlobalMapForMethod(
      ...
      Set<com.debughelper.tools.r8.naming.NamingState<DexProto, ?>> collectedStates,
      Map<Wrapper<DexMethod>, Set<com.debughelper.tools.r8.naming.NamingState<DexProto, ?>>> globalStateMap,
      Map<Wrapper<DexMethod>, Set<DexMethod>> sourceMethodsMap,
      Map<Wrapper<DexMethod>, com.debughelper.tools.r8.naming.NamingState<DexProto, ?>> originStates,
      DexType originInterface) {
    Wrapper<DexMethod> key = equivalence.wrap(method.method);
    Set<com.debughelper.tools.r8.naming.NamingState<DexProto, ?>> stateSet =
  ...
  private void reserveNamesInClasses(DexType type, DexType libraryFrontier,
      com.debughelper.tools.r8.naming.NamingState<DexProto, ?> parent,
      Map<DexType, DexType> frontierMap) {
    assert !type.isInterface();
    DexClass holder = appInfo.definitionFor(type);
    com.debughelper.tools.r8.naming.NamingState<DexProto, ?> state = allocateNamingStateAndReserve(holder, type, libraryFrontier,
        parent, frontierMap);
  ...
  private com.debughelper.tools.r8.naming.NamingState<DexProto, ?> allocateNamingStateAndReserve(DexClass holder, DexType type,
    ...
    com.debughelper.tools.r8.naming.NamingState<DexProto, ?> state =
        computeStateIfAbsent(
            libraryFrontier,
            ignore -> parent == null
                ? com.debughelper.tools.r8.naming.NamingState.createRoot(
  ...
  private void reserveNamesForMethod(DexEncodedMethod method,
      boolean keepAll, com.debughelper.tools.r8.naming.NamingState<DexProto, ?> state) {
  ...
  /**
   * Capture a (name, proto)-pair for a {@link com.debughelper.tools.r8.naming.NamingState}. Each method methodState.METHOD(...)
  ...
  private static class MethodNamingState {
    ...
    private final com.debughelper.tools.r8.naming.NamingState<DexProto, ?> parent;
    private final DexString name;
    private final DexProto proto;

    MethodNamingState(com.debughelper.tools.r8.naming.NamingState<DexProto, ?> parent, DexString name, DexProto proto) {
      assert parent != null;
      this.parent = parent;
      this.name = name;
      this.proto = proto;
