...
public class ReasonPrinter {
  ...
  private final Map<DexEncodedField, com.debughelper.tools.r8.shaking.KeepReason> liveFields;
  private final Map<DexEncodedMethod, com.debughelper.tools.r8.shaking.KeepReason> liveMethods;
  private final Map<DexItem, com.debughelper.tools.r8.shaking.KeepReason> reachablityReasons;
  private final Map<DexType, com.debughelper.tools.r8.shaking.KeepReason> instantiatedTypes;

  ReasonPrinter(Set<DexItem> itemsQueried, Map<DexEncodedField, com.debughelper.tools.r8.shaking.KeepReason> liveFields,
                Map<DexEncodedMethod, com.debughelper.tools.r8.shaking.KeepReason> liveMethods, Map<DexItem, com.debughelper.tools.r8.shaking.KeepReason> reachablityReasons,
                Map<DexType, com.debughelper.tools.r8.shaking.KeepReason> instantiatedTypes) {
  ...
  private void printOnlyAbstractShell(DexItem item, ReasonFormatter formatter) {
    formatter.startItem(item);
    com.debughelper.tools.r8.shaking.KeepReason reachableReason = reachablityReasons.get(item);
  ...
  private void printReasonFor(DexClass item) {
    com.debughelper.tools.r8.shaking.KeepReason reason = instantiatedTypes.get(item.type);
    if (reason == null) {
  ...
  private void printReasonFor(DexEncodedMethod item) {
    com.debughelper.tools.r8.shaking.KeepReason reasonLive = liveMethods.get(item);
    if (reasonLive == null) {
      if (item.accessFlags.isAbstract()) {
        printOnlyAbstractShell(item, formatter);
      } else {
  ...
  private void printReasonFor(DexEncodedField item) {
    com.debughelper.tools.r8.shaking.KeepReason reason = liveFields.get(item);
    if (reason == null) {
  ...
  class ReasonFormatter {
    ...
    void addMethodReferenceReason(DexEncodedMethod method) {
      ...
      startItem(method);
      com.debughelper.tools.r8.shaking.KeepReason reason = reachablityReasons.get(method);
      if (reason != null) {
    ...
    void addTypeLivenessReason(DexType type) {
      ...
      startItem(type);
      pushIsLivePrefix();
      com.debughelper.tools.r8.shaking.KeepReason reason = instantiatedTypes.get(type);
      if (reason != null) {
        reason.print(this);
      }
      popPrefix();
