...
public class SwitchPayloadResolver {
  ...
  private final Map<Integer, com.debughelper.tools.r8.code.SwitchPayload> unresolvedPayload = new HashMap<>();
  private final Map<Integer, PayloadData> payloadToData = new HashMap<>();

  public void addPayloadUser(com.debughelper.tools.r8.code.Instruction dex) {
    ...
    if (unresolvedPayload.containsKey(payloadOffset)) {
      com.debughelper.tools.r8.code.SwitchPayload payload = unresolvedPayload.remove(payloadOffset);
      resolve(payload);
    }
  }
