...
public class DexString extends IndexedDexItem implements PresortedComparable<DexString> {
  ...
  // Inspired from /dex/src/main/java/com/debughelper/dex/Mutf8.java
  ...
  @Override
  public int slowCompareTo(DexString other, com.debughelper.tools.r8.naming.NamingLens lens) {
  ...
  private static boolean isValidClassDescriptor(String string) {
    ...
    for (int i = 1; i < string.length() - 1; i += Character.charCount(cp)) {
      ...
      if (cp != '/' && !com.debughelper.tools.r8.utils.IdentifierUtils.isDexIdentifierPart(cp)) {
        return false;
      }
    }
    return true;
  ...
  private static boolean isValidMethodName(String string) {
    ...
    // According to https://source.debughelper.com/devices/tech/dalvik/dex-format#membername
    ...
    for (int i = 0; i < string.length(); i += Character.charCount(cp)) {
      cp = string.codePointAt(i);
      if (!com.debughelper.tools.r8.utils.IdentifierUtils.isDexIdentifierPart(cp)) {
        return false;
      }
    }
    return true;
