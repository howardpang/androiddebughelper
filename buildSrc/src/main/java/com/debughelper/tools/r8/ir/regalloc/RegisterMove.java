...
public class RegisterMove implements Comparable<RegisterMove> {
  com.debughelper.tools.r8.code.MoveType type;
  int dst;
  int src;
  com.debughelper.tools.r8.ir.code.Instruction definition;

  public RegisterMove(int dst, int src, com.debughelper.tools.r8.code.MoveType type) {
    this.dst = dst;
    this.src = src;
    this.type = type;
  }

  public RegisterMove(int dst, com.debughelper.tools.r8.code.MoveType type, Instruction definition) {
    this.dst = dst;
    this.src = com.debughelper.tools.r8.ir.regalloc.LiveIntervals.NO_REGISTER;
    this.type = type;
  ...
  private boolean writes(int register) {
    if (type == com.debughelper.tools.r8.code.MoveType.WIDE && (dst + 1) == register) {
      return true;
    }
    return dst == register;
  }
